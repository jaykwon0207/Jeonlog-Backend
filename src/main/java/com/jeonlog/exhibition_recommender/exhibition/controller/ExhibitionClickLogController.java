...
public class ExhibitionClickLogController {
    ...
    // 성별별 클릭 통계 조회
    @GetMapping("/click-stats/gender")
    public ResponseEntity<?> getClickStatsByGender() {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        ...
        try {
            Map<String, Object> stats = clickLogService.getClickStatsByGender();
    ...
    // 연령대별 클릭 통계 조회
    @GetMapping("/click-stats/age-group")
    public ResponseEntity<?> getClickStatsByAgeGroup() {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        ...
        try {
            Map<String, Object> stats = clickLogService.getClickStatsByAgeGroup();
            return ResponseEntity.ok(stats);
        } catch (Exception e) {
            return ResponseEntity.status(500).body(Map.of("error", "통계 조회 중 오류가 발생했습니다."));
        }
