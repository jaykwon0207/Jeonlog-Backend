...
public interface ExhibitionClickLogRepository extends JpaRepository<ExhibitionClickLog,Long> {
    ...
    // 성별별 클릭 통계
    @Query("SELECT e.title, u.gender, COUNT(ecl) as clickCount " +
           "FROM ExhibitionClickLog ecl " +
           "JOIN ecl.exhibition e " +
           "JOIN ecl.user u " +
           "WHERE u.gender IS NOT NULL " +
           "GROUP BY e.title, u.gender " +
           "ORDER BY clickCount DESC")
    List<Object[]> findClickStatsByGender();
    ...
    @Query("SELECT e.title, " +
           ...
           "JOIN ecl.exhibition e " +
           "JOIN ecl.user u " +
           "GROUP BY e.title, ageGroup " +
           "ORDER BY clickCount DESC")
    List<Object[]> findClickStatsByAgeGroup();
